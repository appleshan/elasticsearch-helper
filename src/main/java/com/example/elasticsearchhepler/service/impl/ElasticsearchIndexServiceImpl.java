...
public class ElasticsearchIndexServiceImpl<T> implements ElasticsearchIndexService<T> {
    ...
    public Boolean createIndex(Class<T> clazz) throws Exception {
        ...
        //builder.putList("analysis.analyzer.autocomplete.filter",new String[]{"lowercase","autocomplete_filter"});
        // 标准分词后 过滤器再分词
        ...
        logger.info("============索引库indexName：{}创建结果acknowledged：{}", indexName, acknowledged);
        return acknowledged;
    }
