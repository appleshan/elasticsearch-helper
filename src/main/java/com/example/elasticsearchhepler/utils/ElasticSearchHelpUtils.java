...
public class ElasticSearchHelpUtils<T> {
    ...
    public Map<String, Object> objectToMap(T t) {
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                logger.info("实体转map 获取类名：{}中字段：{}的属性值出错,出错原因", t.getClass().getSimpleName(), field.getName(), e);
            }
        }
        ...
        for (Field field : superFields) {
            ...
            try {
                ...
                logger.info("实体转map 获取类名：{}中字段：{}的属性值出错,出错原因", t.getClass().getSuperclass().getSimpleName(),
                        field.getName(), e);
            }
        }
        return map;
    ...
    public int getTotalPages(long totalSize, int pageSize) {
        return totalSize == 0L ? 0 : (Integer.parseInt((
                totalSize % Long.parseLong(pageSize + "") == 0
                        ? totalSize / Long.parseLong(pageSize + "")
                        : totalSize / Long.parseLong(pageSize + "") + 1L
        ) + ""));
    }
}
